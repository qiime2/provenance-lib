...
def _build_header(use):
    """
    A helper that writes usage-specific headers for Replay usage drivers.
    Drivers must have .shebang and .header_boundary properties
    """
    vzn = version('provenance_lib')
    p_lib_md = metadata('provenance_lib')
    ts = datetime.now()

    if hasattr(use, 'shebang'):
        use.header.append(use.shebang)

    use.header.append(use.header_boundary)
    use.header.extend([
        f"# This script auto-generated by provenance_py v.{vzn}",
...
def _build_footer(use, dag: ProvDAG):
