...
class CommentTests(unittest.TestCase):
    """
    Tests confirming that line breaks are handled correctly in `comment()`
    """

    def testCommentWithoutLineBreak(self):
        """
        We expect an empty line after each comment generated by comment()
        """
        input = "Just a flesh wound."
        drivers = ['python3', 'cli',]
        exp = ["# Just a flesh wound.", ""]
        for driver in drivers:
            use: Usage = SUPPORTED_USAGE_DRIVERS[driver]()
            use.comment(input)
            self.assertEqual(exp, use.recorder)
        pass

    def testCommentWithoutLineBreakNotebookCell(self):
        input = "Just a flesh wound."
        exp = input
        use: Usage = SUPPORTED_USAGE_DRIVERS['jn']()
        use.comment(input)
        self.assertEqual(len(use.recorder), 1)
        contents = use.recorder[0]
        self.assertIsInstance(contents, nbf.NotebookNode)
        self.assertEqual(contents['cell_type'], 'markdown')
        self.assertEqual(exp, contents['source'])

    def testCommentWithLineBreak(self):
        input = """'She turned me into a newt.'
'A newt?'
'I got better.'
"""
        drivers = ['python3', 'cli',]
        exp = ["# 'She turned me into a newt.'",
               "# 'A newt?'",
               "# 'I got better.'",
               "",
               ]
    ...
    def testCommentWithLineBreakNotebookCell(self):
        input = """'She turned me into a newt.'
'A newt?'
'I got better.'
"""
        expected = "'She turned me into a newt.'\n'A newt?'\n'I got better.'"
        use: Usage = SUPPORTED_USAGE_DRIVERS['jn']()
        use.comment(input)
        self.assertEqual(len(use.recorder), 1)
        contents = use.recorder[0]
    ...
    def testLongCommentWithIntroducedBreak(self):
        """
        Usage drivers may introduce line breaks to maintain good style
        """
        input = (
    ...
    def testLongCommentWithIntroducedBreakNotebookCell(self):
        input = (
            "I don’t want to talk to you no more, you empty-headed animal "
            "food trough wiper. I fart in your general direction. Your mother"
            " was a hamster and your father smelt of elderberries.")
