...
def build_header(shebang: str = '', boundary: str = '', copyright: str = '',
                 comment_prefix: str = '# ', extra_text: List = []
                 ) -> List[str]:
    """
    ...
    pfx = comment_prefix
    header.extend([
        f"{pfx}Auto-generated by provenance_lib v.{vzn} at "
...
def build_footer(dag: ProvDAG, boundary: str, comment_prefix: str = '# '
    ...
    for idx in range(0, u_len := len(uuids), 2):
        if idx == u_len - 1:
            pairs.append(f"{pfx}{uuids[idx]}")
        else:
            pairs.append(f"{pfx}{uuids[idx]} \t {uuids[idx + 1]}")

    footer.append(boundary)
    footer.append(
        f"{pfx}The following QIIME 2 Results were parsed to produce this"
        " script:")
    footer.extend(pairs)
    footer.append(boundary)
    footer.append('')
...
class ReplayJupyterNotebookUsage(ReplayPythonUsage):
    ...
    how_to = pkg_resources.resource_string(
        __package__, 'assets/jn_howto.txt').decode('utf-8').split('\n')
